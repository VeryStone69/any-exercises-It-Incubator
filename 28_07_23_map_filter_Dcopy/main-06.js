...
const copyUser = {...user, friends: [...user.friends]}; // Object
...
let out = [...students.sort((a, b) => b.scores - a.scores)];
console.log(out)
...
const topstudents = (students.sort((a, b) => b.scores - a.scores)).filter((el, i) => i <= 2);
...
const bestStudent0 = students.reduce((acc, st) => {
    return acc.scores > st.scores ? acc : st;
});
console.log(bestStudent0);
...
for (let i = 1; i < students.length; i++) {
    if (bestStudent1.scores > bestStudent2.scores) {
        if (students[i].scores > bestStudent2.scores) {
            bestStudent2 = students[i];
        }
    } else {
        if (students[i].scores > bestStudent1.scores) {
